...
Handler->>Service : 请求 snapshot_pipeline(...)
Service->>Service : 解析着色器/渲染目标/深度模板/混合/拓扑
...
```mermaid
flowchart TD
Start(["进入 snapshot_pipeline"]) --> SetEvent["设置帧事件"]
SetEvent --> GetState["获取管线状态/API属性"]
GetState --> MapAPI["映射 GraphicsAPI 与 API 特定状态"]
MapAPI --> ExtractShaders["遍历着色器阶段<br/>获取ID与反射"]
...
Handler->>Service : 调用 snapshot_pipeline(...)
Service-->>Handler : 返回 PipelineSnapshot
...
Handler->>Service : snapshot_pipeline(...)
