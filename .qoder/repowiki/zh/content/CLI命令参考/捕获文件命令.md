...
U->>CLI : 传入 --file / --frame-index / --preview / --daemon-context
CLI->>SR : rd.core.init
CLI->>SR : rd.capture.open_file
SR->>RD : 打开捕获文件并探测驱动
SR-->>CLI : 返回 capture_file_id
CLI->>SR : rd.capture.open_replay
SR->>SM : 创建会话本地/远程
...
SR-->>CLI : 返回上下文快照
CLI-->>U : 输出结果含 context_id、capture_file_id、session_id、active_event_id、recovery_status
```
...
```mermaid
flowchart TD
Start(["开始：capture open"]) --> Init["初始化：rd.core.init"]
Init --> OF["打开捕获文件：rd.capture.open_file"]
OF --> OR["打开回放会话：rd.capture.open_replay"]
OR --> SF["设置帧：rd.replay.set_frame"]
SF --> Preview{"是否需要预览？"}
...
V2 --> |通过| BuildSess["构建会话本地/远程"]
BuildSess --> SOK["返回 session_id 与 active_event_id"]
