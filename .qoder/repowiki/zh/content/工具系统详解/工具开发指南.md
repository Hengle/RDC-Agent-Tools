...
graph TB
subgraph "工具开发与运行"
ToolRouter["工具路由<br/>rdx/tool_router.py"]
Engine["执行引擎<br/>rdx/core/engine.py"]
CLI["CLI 入口<br/>cli/run_cli.py"]
Catalog["工具目录规格<br/>spec/tool_catalog.json"]
Overlay["目录叠加层<br/>spec/tool_catalog_overlay.json"]
end
subgraph "文档与规范"
DocsTools["工具开发文档<br/>docs/tools.md"]
DocsQuick["快速开始<br/>docs/quickstart.md"]
DocsInstall["安装指南<br/>docs/install.md"]
DocsConfig["配置说明<br/>docs/configuration.md"]
end
subgraph "发布与质量"
ScriptPack["打包脚本<br/>scripts/package_release.py"]
ScriptSum["校验和生成<br/>scripts/generate_release_checksums.py"]
ScriptVerify["包体校验<br/>scripts/verify_release_package.py"]
Tests["测试套件<br/>tests/*"]
end
subgraph "运行时"
PyRuntime["Python 运行时<br/>rdx/python_runtime.py"]
Boot["运行时引导<br/>rdx/runtime_bootstrap.py"]
Req["运行时需求<br/>rdx/runtime_requirements.py"]
BinPy["Windows x64 Python<br/>binaries/windows/x64/python/python314._pth"]
end
CLI --> ToolRouter
ToolRouter --> Engine
Engine --> PyRuntime
